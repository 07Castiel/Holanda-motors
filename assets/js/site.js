...
(function () {
   ...
   */
  function wppVendasNumero() {
    return cfg.wppVendas || cfg.wpp;
  }
  ...
  function applyConfig() {
    ...
    // Segundo número (vendas) só aparece se estiver preenchido e for diferente
    // do geral — senão a seção mostraria o mesmo telefone duas vezes.
    const vendasLink = document.getElementById('contatoWppVendasLink');
    if (cfg.wppVendas && cfg.wppVendas !== cfg.wpp) {
      vendasLink.href = HM.wppLink('Olá! Vim pelo site e gostaria de saber mais sobre os veículos da Holanda Motors.', cfg.wppVendas);
      vendasLink.textContent = formatPhoneDisplay(cfg.wppVendas);
      vendasLink.style.display = '';
    } else {
      vendasLink.style.display = 'none';
    }
  ...
  function openModalForVehicle(v, triggerEl) {
    ...
    document.getElementById('modalWpp').href = HM.wppLink(wppMsg, wppVendasNumero());
